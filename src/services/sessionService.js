...
export function setupChatSockets(io) {
  io.on('connection', (socket) => {
    logger.info('New user connected to chat');
    ...
    socket.on('chat message', async (msg) => {
      try {
        ...
        // Get username from socket or use default
        const username = socket.username || 'anonBambi';
        ...
        // Check for mentions
        const mentions = detectUserMentions(msg.data);
        if (mentions.length > 0) {
          ...
          // If there are valid mentions, format the message
          if (validatedMentions.some(m => m.exists)) {
            // We'll let the frontend handle the formatting for display
            
            // Notify mentioned users
            const validUsernames = validatedMentions
              .filter(m => m.exists)
              .map(m => m.username);
            ...
            await notifyMentionedUsers(validUsernames, savedMessage, io);
        ...
        // If we reach here, it's a regular message without mentions
        const savedMessage = await ChatMessage.saveMessage(messageData);
        io.emit('chat message', savedMessage);
      } catch (error) {
    ...
    socket.on('set username', (data) => {
      if (data && data.username) {
        ...
        logger.info(`User set username: ${data.username}`);
      }
    });
    ...
    socket.on('triggers', async (data) => {
      if (!data || !data.triggerNames || !Array.isArray(data.triggerNames)) {
        logger.warning('Received invalid trigger format');
        return;
      }
      
      const username = socket.username || 'anonBambi';
      logger.info(`User ${username} activated triggers: ${data.triggerNames.join(', ')}`);
      
      // Emit to all clients
      io.emit('audio triggers', {
        username,
        triggers: data.triggerNames.map(name => ({ name }))
      });
      
      // Log trigger activation if AudioInteraction model exists
      try {
        const AudioInteraction = mongoose.models.AudioInteraction;
        if (AudioInteraction) {
          for (const triggerName of data.triggerNames) {
            await AudioInteraction.create({
              username,
              triggerName,
              timestamp: new Date()
            });
        ...
        logger.error(`Error logging audio interaction: ${error.message}`);
      }
    });
    
    // Handle disconnection
    socket.on('disconnect', () => {
      logger.info(`User disconnected: ${socket.username || 'anonymous'}`);
    });
  });
}
